...
- [x] 29. 初始化小程序项目 `apps/miniapp`
  - 小程序项目需要配置微信 AppID，暂时跳过，后续补充
  - 使用 Taro CLI 创建项目
  - 配置 TypeScript
  - 配置微信小程序 AppID
...
- [x] 32. 检查点 - 验证轨迹功能
  - Web 端手动记录功能已完成，小程序端待配置 AppID 后补充
  - 确保 Web 端手动记录功能正常
  - 确保小程序端实时追踪功能正常
  - 确保轨迹数据正确存储和展示
...
- [x] 42. 实现微信登录接口
  - 已创建 wechat.controller.ts，需配置微信 AppID 后启用
  - 实现 POST `/api/auth/login/wechat` 接口
  - 集成微信 OAuth 获取 openid/unionid
  - 实现微信登录与手机号绑定逻辑

- [x] 43. 实现小程序端微信登录
  - 依赖微信 AppID 配置，小程序端待开发
  - 调用 `wx.login` 获取 code
  - 调用后端接口完成登录
  - 实现 Token 存储和自动刷新
